...
namespace {
  ...
  constexpr int kNumSamples = capusyn::kMaxBufferSize;
  ...
  capusyn::modulation_change createModulationChange(capusyn::ModulationConnection* connection, capusyn::SoundEngine* engine) {
  ...
  void turnEverythingOn(capusyn::SoundEngine* engine) {
    std::map<std::string, capusyn::ValueDetails> parameters = capusyn::Parameters::lookup_.getAllDetails();
...
void ModulationStressTest::processAndCheckFinite(capusyn::Processor* processor) {
...
void ModulationStressTest::allModulations() {
  ...
  capusyn::output_map& source_map = engine.getModulationSources();
  capusyn::input_map& mono_destination_map = engine.getMonoModulationDestinations();
  capusyn::input_map& poly_destination_map = engine.getMonoModulationDestinations();
  ...
  capusyn::ModulationConnectionBank& modulation_bank = engine.getModulationBank();
  ...
  for (int i = 0; i < kModulationHookupNumber; ++i) {
    int source_start = (i * (sources_size - capusyn::kMaxModulationConnections)) / kModulationHookupNumber;
    int dest_start = (i * (destinations_size - capusyn::kMaxModulationConnections)) / kModulationHookupNumber;
    for (int m = 0; m < capusyn::kMaxModulationConnections; ++m) {
      int source_index = capusyn::utils::iclamp(source_start + m, 0, sources_size - 1);
      int destination_index = capusyn::utils::iclamp(dest_start + m, 0, destinations_size - 1);
      std::string source = sources[source_index];
      std::string destination = destinations[destination_index];
      capusyn::ModulationConnection* connection = modulation_bank.createConnection(source, destination);
      ...
      capusyn::modulation_change change = createModulationChange(connection, &engine);
...
void ModulationStressTest::randomModulations() {
  ...
  for (int i = 0; i < kModulationHookupNumber; ++i) {
    ...
    int source_start = (i * (sources_size - capusyn::kMaxModulationConnections / 2)) / kModulationHookupNumber;
    int dest_start = (i * (destinations_size - capusyn::kMaxModulationConnections / 2)) / kModulationHookupNumber;
    for (int m = 0; m < capusyn::kMaxModulationConnections / 2; ++m) {
      int source_index = capusyn::utils::iclamp(source_start + m, 0, sources_size - 1);
      int destination_index = capusyn::utils::iclamp(dest_start + m, 0, destinations_size - 1);
      std::string source = sources[source_index];
      std::string destination = destinations[destination_index];
    ...
    for (int m = capusyn::kMaxModulationConnections / 2; m < capusyn::kMaxModulationConnections; ++m) {
