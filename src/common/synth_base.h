...
class SynthBase : public MidiManager::Listener {
  public:
    ...
    void valueChanged(const std::string& name, capusyn::mono_float value);
    void valueChangedThroughMidi(const std::string& name, capusyn::mono_float value) override;
    void pitchWheelMidiChanged(capusyn::mono_float value) override;
    void modWheelMidiChanged(capusyn::mono_float value) override;
    void pitchWheelGuiChanged(capusyn::mono_float value);
    void modWheelGuiChanged(capusyn::mono_float value);
    void presetChangedThroughMidi(File preset) override;
    void valueChangedExternal(const std::string& name, capusyn::mono_float value);
    void valueChangedInternal(const std::string& name, capusyn::mono_float value);
    bool connectModulation(const std::string& source, const std::string& destination);
    void connectModulation(capusyn::ModulationConnection* connection);
    ...
    capusyn::CircularQueue<capusyn::ModulationConnection*> getModulationConnections() { return mod_connections_; }
    std::vector<capusyn::ModulationConnection*> getSourceConnections(const std::string& source);
    bool isSourceConnected(const std::string& source);
    std::vector<capusyn::ModulationConnection*> getDestinationConnections(const std::string& destination);

    const capusyn::StatusOutput* getStatusOutput(const std::string& name);

    capusyn::Wavetable* getWavetable(int index);
    WavetableCreator* getWavetableCreator(int index);
    capusyn::Sample* getSample();
    ...
    virtual void setValueNotifyHost(const std::string& name, capusyn::mono_float value) { }
    ...
    capusyn::control_map& getControls() { return controls_; }
    capusyn::SoundEngine* getEngine() { return engine_.get(); }
    MidiKeyboardState* getKeyboardState() { return keyboard_state_.get(); }
    const capusyn::poly_float* getOscilloscopeMemory() { return oscilloscope_memory_; }
    const capusyn::StereoMemory* getAudioMemory() { return audio_memory_.get(); }
    const capusyn::StereoMemory* getEqualizerMemory();
    capusyn::ModulationConnectionBank& getModulationBank();
    ...
    struct ValueChangedCallback : public CallbackMessage {
      ValueChangedCallback(std::shared_ptr<SynthBase*> listener, std::string name, capusyn::mono_float val) :
      ...
      capusyn::mono_float value;
    };

  protected:
    capusyn::modulation_change createModulationChange(capusyn::ModulationConnection* connection);
    bool isInvalidConnection(const capusyn::modulation_change& change);
    virtual SynthGuiInterface* getGuiInterface() = 0;
    json saveToJson();
    bool loadFromJson(const json& state);
    capusyn::ModulationConnection* getConnection(const std::string& source, const std::string& destination);

    inline bool getNextModulationChange(capusyn::modulation_change& change) {
    ...
    void processAudioWithInput(AudioSampleBuffer* buffer, const capusyn::poly_float* input_buffer,
    ...
    void updateMemoryOutput(int samples, const capusyn::poly_float* audio);

    std::unique_ptr<capusyn::SoundEngine> engine_;
    std::unique_ptr<MidiManager> midi_manager_;
    std::unique_ptr<MidiKeyboardState> keyboard_state_;

    std::unique_ptr<WavetableCreator> wavetable_creators_[capusyn::kNumOscillators];
    std::shared_ptr<SynthBase*> self_reference_;

    File active_file_;
    capusyn::poly_float oscilloscope_memory_[2 * capusyn::kOscilloscopeMemoryResolution];
    capusyn::poly_float oscilloscope_memory_write_[2 * capusyn::kOscilloscopeMemoryResolution];
    std::unique_ptr<capusyn::StereoMemory> audio_memory_;
    capusyn::mono_float last_played_note_;
    int last_num_pressed_;
    capusyn::mono_float memory_reset_period_;
    capusyn::mono_float memory_input_offset_;
    int memory_index_;
    bool expired_;

    std::map<std::string, String> save_info_;
    capusyn::control_map controls_;
    capusyn::CircularQueue<capusyn::ModulationConnection*> mod_connections_;
    moodycamel::ConcurrentQueue<capusyn::control_change> value_change_queue_;
    moodycamel::ConcurrentQueue<capusyn::modulation_change> modulation_change_queue_;
    Tuning tuning_;
