...
void FrequencyFilterModifier::FrequencyFilterModifierKeyframe::render(capusyn::WaveFrame* wave_frame) {
...
float FrequencyFilterModifier::FrequencyFilterModifierKeyframe::getMultiplier(float index) {
  ...
  float slope = 1.0f / capusyn::utils::interpolate(1.0f, kMaxSlopeReach, shape_ * shape_);
  float power = capusyn::utils::interpolate(kMinPower, kMaxPower, shape_);

  if (style_ == kLowPass)
    return capusyn::utils::clamp(1.0f - slope * cutoff_delta, 0.0f, 1.0f);
  if (style_ == kBandPass)
    return capusyn::utils::clamp(1.0f - fabsf(slope * cutoff_delta), 0.0f, 1.0f);
  if (style_ == kHighPass)
    return capusyn::utils::clamp(1.0f + slope * cutoff_delta, 0.0f, 1.0f);
  if (style_ == kComb)
...
void FrequencyFilterModifier::render(capusyn::WaveFrame* wave_frame, float position) {
