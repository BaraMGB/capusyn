...
float WaveWindowModifier::applyWindow(WindowShape window_shape, float t) {
  if (window_shape == kCos)
    return 0.5f - 0.5f * cosf(capusyn::kPi * t);
  if (window_shape == kHalfSin)
    return sinf(capusyn::kPi * t / 2.0f);
  if (window_shape == kSquare)
    return t < 1.0f ? 0.0f : 1.0f;
  if (window_shape == kWiggle)
    return t * cosf(capusyn::kPi * (t * 1.5f + 0.5f));
  return t;
}
...
void WaveWindowModifier::WaveWindowModifierKeyframe::render(capusyn::WaveFrame* wave_frame) {
  ...
  for (int i = capusyn::WaveFrame::kWaveformSize; i >= 0; --i) {
    float t = i / (capusyn::WaveFrame::kWaveformSize - 1.0f);
...
void WaveWindowModifier::render(capusyn::WaveFrame* wave_frame, float position) {
