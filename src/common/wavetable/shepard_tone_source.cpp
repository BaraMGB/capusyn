...
void ShepardToneSource::render(capusyn::WaveFrame* wave_frame, float position) {
  if (numFrames() == 0)
    return;

  WaveSourceKeyframe* keyframe = getKeyframe(0);
  capusyn::WaveFrame* key_wave_frame = keyframe->wave_frame();
  capusyn::WaveFrame* loop_wave_frame = loop_frame_->wave_frame();
  ...
  compute_frame_->interpolate(keyframe, loop_frame_.get(), position / (capusyn::kNumOscillatorWaveFrames - 1.0f));
