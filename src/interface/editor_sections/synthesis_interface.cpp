...
SynthesisInterface::SynthesisInterface(Authentication* auth,
                                       ...
                                       const capusyn::output_map& poly_modulations) : SynthSection("synthesis") {
...
void SynthesisInterface::resized() {
  ...
  int sample_y = oscillators_[capusyn::kNumOscillators - 1]->getBottom() + padding;
...
void SynthesisInterface::distortionTypeChanged(OscillatorSection* section, int distortion_type) {
  bool dependents[capusyn::kNumOscillators];
  for (int i = 0; i < capusyn::kNumOscillators; ++i)
    dependents[i] = false;
  ...
  while (!dependents[index]) {
    ...
    if (capusyn::SynthOscillator::isFirstModulation(type))
      index = capusyn::ProducersModule::getFirstModulationIndex(index);
    else if (capusyn::SynthOscillator::isSecondModulation(type))
      index = capusyn::ProducersModule::getSecondModulationIndex(index);
    else
      return;
  }
...
void SynthesisInterface::oscillatorDestinationChanged(OscillatorSection* section, int destination) {
  bool filter1_on = destination == capusyn::constants::kFilter1 || destination == capusyn::constants::kDualFilters;
  bool filter2_on = destination == capusyn::constants::kFilter2 || destination == capusyn::constants::kDualFilters;
  for (int i = 0; i < capusyn::kNumOscillators; ++i) {
