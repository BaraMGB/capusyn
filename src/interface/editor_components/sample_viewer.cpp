...
void SampleViewer::setLinePositions() {
  ...
  const capusyn::mono_float* buffer = sample_->buffer();
...
void SampleViewer::render(OpenGlWrapper& open_gl, bool animate) {
  ...
  capusyn::poly_float encoded_phase = sample_phase_output_->value();
  ...
  capusyn::poly_mask switch_mask = capusyn::poly_float::notEqual(voice, last_voice_);
  capusyn::poly_float phase_reset = capusyn::utils::max(0.0f, phase);
  last_phase_ = capusyn::utils::maskLoad(last_phase_, phase_reset, switch_mask);

  if (!sample_phase_output_->isClearValue(phase) && capusyn::poly_float::notEqual(phase, 0.0f).anyMask() != 0) {
